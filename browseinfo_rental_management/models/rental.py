...
class AccountInvoiceLine(models.Model):
    ...
    sale_rental_line_ids = fields.Many2many('sale.rental.order.line', string='Sale Rental Order Lines', readonly=True,
                                            copy=False)
...
class MailTemplate(models.Model):
    ...
    def send_mail(self, res_id, force_send=False, raise_exception=False, email_values=None):
        ...
        if self._context.get('auther'):
            self.env['mail.mail'].sudo().browse(res).author_id = self._context.get(
                'auther').id  # [(6,0,[self._context.get('attachment').id])]
        return res
...
class RentalOrder(models.Model):
    ...
    def check_contract(self):
        ...
        for record in self.search([('state', '=', 'confirm')]):
            ...
            if record.rental_bill_freq_type == 'hours':
                ...
                if record.renew_date == today_date and today_date <= record.end_date:
                    record.sudo()._create_invoice()
    ...
    origin = fields.Char(string='Source Document',
                         help="Reference of the document that generated this rental orders request.")
    ...
    company_id = fields.Many2one('res.company', 'Company',
                                 default=lambda self: self.env['res.company']._company_default_get('rental.order'))
    ...
    picking_ids = fields.Many2many('stock.picking', compute='_compute_picking_ids',
                                   string='Picking associated to this Rental')
    ...
    def _compute_picking_ids(self):
        ...
        for order in self:
            order.picking_ids = self.env['stock.picking'].search(
                [('group_id', '=', order.procurement_group.id)]) if order.procurement_group else []
    ...
    def _onchange_rental_initial_type(self):
        for rental in self:
            ...
            if rental.rental_initial_type == 'hours':
                rental.rental_bill_freq_type = 'hours'
        ...
        for rental in self:
            ...
            picking_type_id = self.env['stock.picking.type'].search([('code', '=', 'outgoing'),
                                                                     ('warehouse_id.company_id', '=',
                                                                      rental.company_id.id)], limit=1)

            for line in rental.sale_line:
                if line.product_id.type in ['consu', 'product']:
                    location_search_id = self.env['stock.location'].search(
                        [('usage', '=', 'internal'), ('company_id', '=', rental.company_id.id)])
...
class SaleRentalOrderLine(models.Model):
    ...
    rental_id = fields.Many2one('rental.order', string='Rental Reference', required=True, ondelete='cascade',
    ...
    def _action_launch_procurement_rule_custom(self):
        ...
        for order in orders:
            ...
            if reassign:
                ...
                move = reassign.move_line_ids.write({'lot_id': line.lot_id.id})
            ...
            if move.location_id.usage in ('supplier', 'inventory', 'production', 'customer') \
                ...
                if move.product_id.tracking == 'serial' and (
                    ...
                    if to_update:
                        ...
                        abc = self.env['stock.move.line'].create(
